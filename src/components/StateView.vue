<template>
  <div class="state mb-4 mt-3 row text-center">
    <div class="col-md-12">
      <h2>
        State <span v-if="state['runType']">({{ state["runType"] }})</span><span v-if="state['strategy']">, Strategy {{ strategy }}</span>
      </h2>
      <div class="narration">
        ...
        >
          <div class="suit-label-small" :class="item['suit']" />
          <span> is playing </span>
          <span>{{ getCard(item["card"]["card"]) }} of </span>
          <div class="suit-label-small" :class="item['card']['suit']" />
          <span>. </span>
        </div>
      </div>
    </div>
    <div class="suit row">
      <div class="col-md-12">
        <div v-for="(cards, suit) in state.suits" :key="suit" :class="suit">
          <div class="suit-label" :class="suit" />
          ...
          <div v-if="!state.suits[suit].blocked" class="blocked">
            No
          </div>
          <div v-if="state.suits[suit].blocked" class="blocked">
            Yes
